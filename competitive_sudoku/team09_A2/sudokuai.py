...
class SudokuAI(competitive_sudoku.sudokuai.SudokuAI):
    ...
    def get_filled_block_values(self,row_index: int, column_index: int, game_state: GameState):
        ...
        first_column = (column_index // game_state.board.n) * game_state.board.n
        filled_values = []
    ...
    def get_illegal_moves(self,row_index: int, col_index: int, game_state: GameState):
        ...
        """
        illegal = self.get_filled_row_values(row_index, game_state) + self.get_filled_column_values(col_index, game_state) + self.get_filled_block_values(row_index, col_index, game_state)
    ...
    def evaluate_move_score_increase(self,move: Move, game_state: GameState, allow_recusion=True):
        ...
        if allow_recusion:
            ...
            if is_row_almost_filled:
                ...
                move_score = self.evaluate_move_score_increase(Move(empty_cell_index[0], empty_cell_index[1], missing_value), game_state, False)
                potential_row_move_points_lost = move_score
                # Remove move from board to return to original game_state
                game_state.board.put(empty_cell_index[0], empty_cell_index[1], SudokuBoard.empty)
            else:
    ...
    def get_empty_cells(self,game_state: GameState):
        ...
        empty_cells = [(i,j) for i in self.range_N for j in self.range_N if game_state.board.get(i, j) == SudokuBoard.empty]
        return empty_cells
    ...
    def find_optimal_move(self,game_state: GameState, max_depth):
        ...
        # find all possible legal moves for the current game game_state
        ...
        for legal_move in legal_moves:
            ...
            score_increase = self.evaluate_move_score_increase(legal_move, game_state)
            # Make the move
            ...
            if game_state.scores:
                ...
                game_state.scores = [0, score_increase]
            cur_max_score = self.minimax(game_state, max_depth, 0, -math.inf, math.inf, False)
    ...
    def get_greedy_move(self, game_state: GameState, legal_moves):
        ...
        @param game_state: The GameState object that describes the current game_state of the game in progress
        @param legal_moces: All legal moves that can be performed given the current game_state of the game
    ...
    def minimax(self, game_state: GameState, max_depth: int, depth: int, alpha: float, beta: float, is_maximizing_player: bool):
        """
        ...
        if len(legal_moves) == 0:
            ...
            # between the score of the maximizer at this game_state and the score of the minimizer at the same game_state.
        ...
        if is_maximizing_player:
            ...
            for legal_move in legal_moves:
                ...
                # legal_move
                maximizer_score_increase = self.evaluate_move_score_increase(legal_move, game_state)
                ...
                max_score = max(max_score, self.minimax(game_state, max_depth, depth+1, alpha, beta, False))
            ...
            for legal_move in legal_moves:
                ...
                # legal_move
                minimizer_score_increase = self.evaluate_move_score_increase(legal_move, game_state)
                ...
                min_score = min(min_score, self.minimax(game_state, max_depth, depth+1, alpha, beta, True))
        ...
        for i in self.range_N:
            for j in self.range_N:
                for value in self.range_N_plus_1:
                    if self.is_possible(i, j, value, game_state) and value not in self.get_illegal_moves(i, j, game_state):
        ...
        while True:
            ...
            if best_move != Move(-1, -1, -1):  # Failsafe mechanism to ensure we will never propose an invalid move
                if self.verbose:
                    ...
                    print("Score for selected legal_move: " + str(self.evaluate_move_score_increase(best_move, game_state)))
                    print("Illegal moves for selected cell: " + str(self.get_illegal_moves(best_move.i, best_move.j, game_state)))
                    print("Block filled values for selected cell: " + str(self.get_filled_block_values(best_move.i, best_move.j, game_state)))
                    print("Row filled values for selected cell: " + str(self.get_filled_row_values(best_move.i, game_state)))
                    print("Column filled values for selected cell: " + str(self.get_filled_column_values(best_move.j, game_state)))
                    print("--------------")

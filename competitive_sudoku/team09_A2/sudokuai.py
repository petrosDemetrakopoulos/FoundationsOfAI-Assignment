...
class SudokuAI(competitive_sudoku.sudokuai.SudokuAI):
    ...
    def compute_best_move(self, game_state: GameState) -> None:
        ...
        def evaluate_move_score_increase(move: Move, game_state: GameState, allow_recusion=True):
            ...
            filled_col = get_filled_column_values(move.j, game_state)
            ...
            if allow_recusion:
                ...
                if is_row_almost_filled:
                    ...
                    empty_cell_index = [x for x in get_empty_cells(game_state) if x[0] == move.i][0]
                    ...
                    move_score = evaluate_move_score_increase(Move(empty_cell_index[0], empty_cell_index[1], missing_value), game_state, False)
                ...
                if is_col_almost_filled:
                    ...
                    empty_cell_index = [x for x in get_empty_cells(game_state) if x[1] == move.j][0]
                    # Place move that is immediately available for point scoring
                    game_state.board.put(empty_cell_index[0], empty_cell_index[1], missing_value)
                    # Evaluate that move to check how many points it awards
                    move_score = evaluate_move_score_increase(Move(empty_cell_index[0], empty_cell_index[1], missing_value), game_state, False)
                ...
                if is_block_almost_filled:
                    ...
                    empty_cell_index = [x for x in get_empty_cells(game_state) if x[0] in range(first_row, first_row + game_state.board.m) and x[1] in range(first_column, first_column + game_state.board.n)][0]
                    # Place move that is immediately available for point scoring
                    game_state.board.put(empty_cell_index[0], empty_cell_index[1], missing_value)
                    # Evaluate that move to check how many points it awards
                    move_score = evaluate_move_score_increase(Move(empty_cell_index[0], empty_cell_index[1], missing_value), game_state, False)
        ...
        def legal_moves_after_pruning(game_state: GameState, empty_cells):
            ...
            if not game_state.board.empty:
                for cell in empty_cells:
                    ...
                    if not (len(get_filled_row_values(row_index, game_state)) == 0 and
                            len(get_filled_column_values(cell_index, game_state)) == 0 and
                            len(get_filled_block_values(row_index, cell_index, game_state)) == 0):
            ...
            for coords in known_no_reward_cells:
                for value in range_N_plus_1:
                    if possible(coords[0], coords[1], value) and value not in get_illegal_moves(coords[0], coords[1], game_state):
        ...
        def get_empty_cells(game_state: GameState):
            ...
            empty_cells = [(i,j) for i in range_N for j in range_N if game_state.board.get(i, j) == SudokuBoard.empty]
            return empty_cells

        # the function that initially triggers the recursion
        def find_optimal_move(game_state: GameState, max_depth):
            """
            ...
            legal_moves = legal_moves_after_pruning(game_state, empty_cells)

            for legal_move in legal_moves:
                # Calculate the amount by which the score of the maximizing player will be increased if it plays legal_move
                score_increase = evaluate_move_score_increase(legal_move, game_state)
                # Make the move
                ...
                if game_state.scores:
                    ...
                    game_state.scores = [0, score_increase]
                cur_max_score = minimax(game_state, max_depth, 0, -math.inf, math.inf, False)
        ...
        def get_greedy_move(game_state: GameState, legal_moves):
            """
        ...
        def minimax(game_state: GameState, max_depth: int, depth: int, alpha: float, beta: float, is_maximizing_player: bool):
            """
            ...
            if is_maximizing_player:
                ...
                for legal_move in legal_moves:
                    ...
                    # legal_move
                    maximizer_score_increase = evaluate_move_score_increase(legal_move, game_state)
                    ...
                    max_score = max(max_score, minimax(game_state, max_depth, depth+1, alpha, beta, False))
                ...
                for legal_move in legal_moves:
                    ...
                    # legal_move
                    minimizer_score_increase = evaluate_move_score_increase(legal_move, game_state)
                    ...
                    min_score = min(min_score, minimax(game_state, max_depth, depth+1, alpha, beta, True))
